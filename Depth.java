...
public class Depth {
	...
	public static void main(String[] args) {
		...
		Deque<Node> c_stack = new ArrayDeque<>();
		...
		while(!stack.isEmpty()){
			...
			if(checkNode == goalNode) {
				...
				c_stack.push(checkNode);
				for(int i = 0 ;i < checkNode.children.size(); i++) {
					if(stack.contains(checkNode.children.get(i)) == false && c_stack.contains(checkNode.children.get(i)) == false) {
						checkNode.children.get(i).setPointer(checkNode);
					}
					stack.push((Node) checkNode.children.get(i));
				}
			}
		} 
